...
class Model:
    ...
    def train(self, training_file = None, instances = None, save_as = None, folds = 10):
        ...
        process = subprocess.Popen(["java", "-cp", "./weka.jar", "-Xmx"+str(self.max_memory)+"M", "weka.classifiers."+self.classifier, "-x", str(folds),"-t", self.training_file, "-d", save_as], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def test(self, test_file = None, instances = None, model_file = None):       
        ...
        process = subprocess.Popen(["java", "-cp", "./weka.jar", "-Xmx"+str(self.max_memory)+"M", "weka.classifiers."+self.classifier, "-T", self.test_file, "-l", self.model_file, "-p", "0"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
